...
def main1(id):
    ...
    if request.method=='POST':
        calorie = request.form['calIn']
        return redirect(f"/result/{obj.id}/{calorie}")
    return render_template("main.html",obj=obj)

@app.route("/result/<int:id>/<calo>", methods=['GET','POST'])   
def result(id,calo):
    obj = User.query.filter_by(id=id).first()
    ht = obj.height
    wt = obj.weight
    ...
    calo=calo
    if(gender == "Male"):
        cal = round((10*wt + 6.25*ht - 5*age + 5)*1.375,2)
    else:
        cal = round((10*wt + 6.25*ht - 5*age - 161)*1.375,2)
    carb = round(0.45*cal,2)
    fat = round(0.30*cal,2)
    protein = round(0.25*cal,2)
    bmi = round(((wt/(ht*ht))*10000),2)
    return render_template("result.html",cal=cal,carb=carb,fat=fat,protein=protein,bmi=bmi,calo=calo)
